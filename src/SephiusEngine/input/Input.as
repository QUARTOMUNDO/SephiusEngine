package SephiusEngine.input {
	...
	public class Input{
		...
		public function get actions():Vector.<InputAction> { return _actions; }
		public var actionsByChannelByName:Dictionary = new Dictionary();
		public var actionsByNameByChannel:Dictionary = new Dictionary();
		public var numActionByChannel:Dictionary = new Dictionary();
		public var numActionByName:Dictionary = new Dictionary();
		...
		public function addAction(action:InputAction):void{
			if (_actions.lastIndexOf(action) < 0) {
				if (!actionsByChannelByName[action.channel])
					actionsByChannelByName[action.channel] = new Dictionary();
				
				if (!actionsByNameByChannel[action.name]){
					actionsByNameByChannel[action.name] = new Dictionary();
					numActionByName[action.name] = 0;
				}
				
				if(!actionsByNameByChannel[action.name][action.channel])
					numActionByName[action.name]++;
				
				_actions[_actions.length] = action;
				actionsByChannelByName[action.channel][action.name] = action;
				actionsByNameByChannel[action.name][action.channel] = action;
		...
		internal function doActionON(action:InputAction):void{
			...
			if(!actionsByNameByChannel[action.name][action.channel]){
				numActionByName[action.name]++;
				//trace("INPUTON: " + action.name + " / " + action.channel);
			}
		...
		internal function doActionOFF(action:InputAction):void{
			...
			if(!actionsByNameByChannel[action.name][action.channel]){
				numActionByName[action.name]++;
				trace("INPUTOFF: " + action.name + " / " + action.channel);
			}
		...
		internal function doActionCHANGE(action:InputAction):void{
			...
			if(!actionsByNameByChannel[action.name][action.channel]){
				numActionByName[action.name]++;
				//trace("INPUTCHANGE: " + action.name + " / " + action.channel);
			}
			
			actionsByChannelByName[action.channel][action.name] = action;
			actionsByNameByChannel[action.name][action.channel] = action;
			//trace("INPUTCHANGE: " + actionsByNameByChannel[action.name][action.channel].name);
		}
		...
		public function update():void{
			...
			for (i in _actions){
				...
				if (_actions[i].phase > InputPhase.END){
					//trace("INPUTDELETE: " + _actions[i].name + " / " + _actions[i].channel + " / " + numActionByName[_actions[i].name]--);
					_actions[i].dispose();
					numActionByName[_actions[i].name]--
					if (numActionByName[_actions[i].name] <= 0) {
						delete actionsByChannelByName[_actions[i].channel][_actions[i].name];
						delete actionsByNameByChannel[_actions[i].name][_actions[i].channel];
						delete actionsByNameByChannel[_actions[i].name];
						delete numActionByName[_actions[i].name];
						//trace("ACTIONREMOVE: " + _actions[i].name + " / " + _actions[i].channel);
		...
		public function resetActions():void{
			_actions.length = 0;
			actionsByChannelByName = new Dictionary();
			actionsByNameByChannel = new Dictionary();
			actionsByNameByChannel = new Dictionary();
		}
		...
		private function destroyControllers():void{
			...
			actionsByChannelByName = new Dictionary();
			actionsByNameByChannel = new Dictionary();
			numActionByName = new Dictionary();
		}
