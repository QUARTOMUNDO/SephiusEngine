...
package starling.display
{
    ...
    public class MovieClip extends Image implements IAnimatable
    {
		...
		public function clone():MovieClip {
			var clone:MovieClip = new MovieClip(mTextures, fps, false);
			clone.initClone(mTextures, mSounds, mDurations, mStartTimes);
		...
		public function initClone(textures:Vector.<Texture>, mSounds:Vector.<Sound>, mDurations:Vector.<Number>, mStartTimes:Vector.<Number>):void {
			this.mDefaultFrameDuration = mDefaultFrameDuration;
			
			this.mSounds = mSounds.concat();
			this.mDurations = mDurations.concat();
			this.mStartTimes = mStartTimes.concat();
        ...
        public function MovieClip(textures:Vector.<Texture>, fps:Number=12, initMC:Boolean = true)
        {
        ...
        {
			if (1.0 / mDefaultFrameDuration == value)
				return;
