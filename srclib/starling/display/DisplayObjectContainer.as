...
package starling.display
{
    ...
    public class DisplayObjectContainer extends DisplayObject
    {
		...
		private var rcaChild:DisplayObject;
		private var rcaContainer:DisplayObjectContainer;
        ...
        public function removeChildAt(index:int, dispose:Boolean=false):DisplayObject
        {
            if (index >= 0 && index < numChildren)
            {
                rcaChild = mChildren[index];
                rcaChild.dispatchEventWith(Event.REMOVED, true);
                
                if (stage)
                {
                    rcaContainer = rcaChild as DisplayObjectContainer;
                    if (rcaContainer) rcaContainer.broadcastEventWith(Event.REMOVED_FROM_STAGE);
                    else           rcaChild.dispatchEventWith(Event.REMOVED_FROM_STAGE);
                }
                
                rcaChild.setParent(null);
                index = mChildren.indexOf(rcaChild); // index might have changed by event handler
                if (index >= 0) mChildren.splice(index, 1); 
                if (dispose) rcaChild.dispose();
        ...
        public function removeChildren(beginIndex:int=0, endIndex:int=-1, dispose:Boolean=false):void
        {
            ...
            for (rcI=beginIndex; rcI<=endIndex; ++rcI)
        ...
        {
            hNumChildren = mChildren.length;
            for (rcI=0; rcI<hNumChildren; ++rcI)
                if (mChildren[rcI].name == name) return mChildren[rcI];
		...
		private var rcI2:int;
        /** Swaps the indexes of two children. */
        public function swapChildren(child1:DisplayObject, child2:DisplayObject):void
        {
            rcI = getChildIndex(child1);
            rcI2 = getChildIndex(child2);
            if (rcI == -1 || rcI2 == -1) throw new ArgumentError("Not a child of this container");
            swapChildrenAt(rcI, rcI2);
        }
		
		private var scChild2:DisplayObject;
        /** Swaps the indexes of two children. */
        public function swapChildrenAt(index1:int, index2:int):void
        {
            rcaChild = getChildAt(index1);
            scChild2 = getChildAt(index2);
            mChildren[index1] = scChild2;
            mChildren[index2] = rcaChild;
        }
        ...
        // other methods
        private var hNumChildren:int;
		private var minX:Number;
		private var minY:Number;
		private var maxX:Number;
		private var maxY:Number;
		private var gbI:Number;
        /** @inheritDoc */ 
        public override function getBounds(targetSpace:DisplayObject, resultRect:Rectangle=null):Rectangle
        {
            ...
            else
            {
                ...
                for (gbI=0; gbI<hNumChildren; ++gbI)
                {
                    mChildren[gbI].getBounds(targetSpace, resultRect);
        ...
        private var hChild:DisplayObject;
		private var hFilter:FragmentFilter; 
		private var hI:uint; 
		private var hAlpha:Number; 
		private var hBlendMode:String; 
        /** @inheritDoc */
        public override function render(support:RenderSupport, parentAlpha:Number):void
        {
           hAlpha = parentAlpha * this.alpha;
           hNumChildren = mChildren.length;
           hBlendMode = support.blendMode;
			
            for (hI=0; hI<hNumChildren; ++hI)
            {
                hChild = mChildren[hI];
                
                if (hChild.hasVisibleArea)
                {
                    hFilter = hChild.filter;
                    support.pushMatrix();
                    support.transformMatrix(hChild);
                    support.blendMode = hChild.blendMode;
                    
                    if (hFilter) hFilter.render(hChild, support, hAlpha);
                    else        hChild.render(support, hAlpha);
                    
                    support.blendMode = hBlendMode;
                    support.popMatrix();
                }
            }
        ...
        public function broadcastEvent(event:Event):void
        {
            ...
            for (bI=fromIndex; bI<toIndex; ++bI)
                sBroadcastListeners[bI].dispatchEvent(event);
