...
package starling.display
{
    ...
    public class Sprite extends DisplayObjectContainer
    {
		...
		private var hClipRect:Rectangle;
		private var mvpMatrix:Matrix3D;
		private var mAlpha:Number;
		private var hNumBatches:int;
		private var hQuadBatch:QuadBatch;
        ...
        public override function render(support:RenderSupport, parentAlpha:Number):void
        {
            if (mClipRect)
            {
                hClipRect = support.pushClipRect(getClipRect(stage, sHelperRect));
                if (hClipRect.isEmpty())
                {
            ...
            if (mFlattenedContents || mFlattenRequested)
            {
                ...
                mAlpha = parentAlpha * this.alpha;
                hNumBatches = mFlattenedContents.length;
                mvpMatrix = support.mvpMatrix3D;
                
                support.finishQuadBatch();
                support.raiseDrawCount(hNumBatches);
                
                for (hI=0; hI<hNumBatches; ++hI)
                {
                    hQuadBatch = mFlattenedContents[hI];
                    hBlendMode = hQuadBatch.blendMode == BlendMode.AUTO ?
                        support.blendMode : hQuadBatch.blendMode;
                    hQuadBatch.renderCustom(mvpMatrix, mAlpha, hBlendMode);
                }
            }
