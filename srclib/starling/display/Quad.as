...
package starling.display
{
    ...
    public class Quad extends DisplayObject
    {
        ...
        private var hScaleX:Number = this.scaleX;
        private var hScaleY:Number = this.scaleY;
        /** @inheritDoc */
        public override function getBounds(targetSpace:DisplayObject, resultRect:Rectangle=null):Rectangle
        {
            ...
            else if (targetSpace == parent && rotation == 0.0) // optimization
            {
                ...
                hScaleY = this.scaleY;
                mVertexData.getPosition(3, sHelperPoint);
                resultRect.setTo(x - pivotX * hScaleX,      y - pivotY * hScaleY,
                                 sHelperPoint.x * hScaleX, sHelperPoint.y * hScaleY);
                if (hScaleX < 0) { resultRect.width  *= -1; resultRect.x -= resultRect.width;  }
                if (hScaleY < 0) { resultRect.height *= -1; resultRect.y -= resultRect.height; }
            }
            else if (is3D && stage)
            {
        ...
        /** Returns the color of a vertex at a certain index. */
        public function setVetexPosition(vertexID:int, posX:Number, posY:Number):void{
            return mVertexData.setPosition(vertexID, posX, posY);
        }
        ...
        /** Sets the color of a vertex at a certain index. */
        public function multiplyVertexColor(vertexID:int, color:uint):void
        {
            mVertexData.multiplyColor(vertexID, color);
            onVertexDataChanged();
