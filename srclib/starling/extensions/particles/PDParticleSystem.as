...
package starling.extensions.particles
{
    ...
    {
		private var _repelentForce:Number = 20;
		private var _attractorForce:Number = -150;
        ...
        protected override function advanceParticle(aParticle:Particle, passedTime:Number):void
        {
            ...
            else
            {
				...
				var repelentDistanceX:Number = particle.x - emitterX;
				var repelentDistanceY:Number = particle.y - emitterY;
				var repelentDistanceScalar:Number = Math.sqrt(repelentDistanceX*repelentDistanceX + repelentDistanceY*repelentDistanceY);
				if (repelentDistanceScalar < 0.01) repelentDistanceScalar = 0.01;
				
                var repelentRadialX:Number = repelentDistanceX / repelentDistanceScalar;
                var repelentRadialY:Number = repelentDistanceY / repelentDistanceScalar;
				
                repelentRadialX *= repelentForce;
                repelentRadialY *= repelentForce;
				
				if(_attractor){
					var attractorDistanceX:Number = particle.x - (_attractor.inverted ? _attractor.x - 50 : _attractor.x + 50);
					var attractorDistanceY:Number = particle.y - _attractor.y;
					var attractorDistanceScalar:Number = Math.sqrt(attractorDistanceX*attractorDistanceX + attractorDistanceY*attractorDistanceY);
					if (attractorDistanceScalar < 0.01) attractorDistanceScalar = 0.01;
					
					var attractorRadialX:Number = attractorDistanceX / (attractorDistanceScalar * attractorDistanceScalar);
					var attractorRadialY:Number = attractorDistanceY / (attractorDistanceScalar * attractorDistanceScalar);
					
					attractorRadialX *= _attractorForce * 500;
					attractorRadialY *= _attractorForce * 500;
					
					particle.scale -= (particle.scale > 0) ? (0.15 / (attractorDistanceScalar) * ((particle.scale + 1) * (particle.scale + 1))) : 0;
					
					if (attractorDistanceScalar < 50) {
						particle.colorArgb.alpha -= 0.1;
						if (particle.colorArgb.alpha < 0.05){
							var lastParticle:Particle = mParticles[int(mNumParticles-1)] as Particle;
							mParticles[int(mNumParticles - 1)] = aParticle;
							mParticles[mParticles.indexOf(aParticle)] = lastParticle;
							//trace("particle aborbed", mParticles.indexOf(aParticle));
							--mNumParticles;
						}
					}
				}else {
                ...
                particle.velocityX += passedTime * (mGravityX + radialX + tangentialX + attractorRadialX + repelentRadialX);
                particle.velocityY += passedTime * (mGravityY + radialY + tangentialY + attractorRadialY + repelentRadialY);
		...
		public function get repelentForce():Number{ return _repelentForce; }
		public function set repelentForce(value:Number):void { _repelentForce = value; }
		
		public function get attractorForce():Number{ return _attractorForce; }
		public function set attractorForce(value:Number):void { _attractorForce = value; }
		
		public function get attractor():Object{return _attractor;}
		public function set attractor(value:Object):void {_attractor = value; trace(value)}
    }
}
