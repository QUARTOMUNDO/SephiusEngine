...
package starling.utils
{
    ...
    public class VertexData 
    {
        ...
        /** Multiplies the RGB color values of a vertex with a new color (alpha is not changed). */
        public function multiplyColor(vertexID:int, color:uint):void
        {
            var offset:int = vertexID * ELEMENTS_PER_VERTEX + COLOR_OFFSET;
            var multiplier:Number = mPremultipliedAlpha ? mRawData[int(offset+3)] : 1.0;

            // Retrieve existing color components
            var currentRed:Number = mRawData[offset] * 255.0;
            var currentGreen:Number = mRawData[int(offset+1)] * 255.0;
            var currentBlue:Number = mRawData[int(offset+2)] * 255.0;

            // Extract new color components
            var newRed:Number = ((color >> 16) & 0xff);
            var newGreen:Number = ((color >> 8) & 0xff);
            var newBlue:Number = (color & 0xff);

            // Multiply and normalize color components
            mRawData[offset]        = (currentRed * newRed / 255.0) / 255.0 * multiplier;
            mRawData[int(offset+1)] = (currentGreen * newGreen / 255.0) / 255.0 * multiplier;
            mRawData[int(offset+2)] = (currentBlue * newBlue / 255.0) / 255.0 * multiplier;
        }
        ...
        /** Returns the RGB color of a vertex (no alpha). */
        public function getColorsNormalized(vertexID:int):Vector.<Number>{
            var offset:int = vertexID * ELEMENTS_PER_VERTEX + COLOR_OFFSET;
            var divisor:Number = mPremultipliedAlpha ? mRawData[int(offset+3)] : 1.0;
            
            if (divisor == 0) return Vector.<Number>(1, 1, 1);
                ...
                return Vector.<Number>([red, green, blue]);
            }
        }
